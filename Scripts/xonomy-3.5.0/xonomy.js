...
Xonomy.isNamespaceDeclaration = function (attributeName) {
	...
	if (attributeName.length >= 6 && attributeName.substring(0, 6) == "xmlnxs:") ret = true;
	return ret;
};
Xonomy.namespaces = {}; //eg. "xmlnxs:mbm": "http://lexonista.com"
...
Xonomy.refresh = function () {
	...
	$(".xonomy .attributes").each(function () { //determine whether each attribute list has any shy attributexs:
	...
	$(".xonomy .element").each(function () { //refresh display names, display values and captionxs:
...
Xonomy.askDateString = function (defaultString, askerParameter, jsMe) {
	var width = ($("body").width() * .5) - 575
	var html = "";
	html += "<form onsubmit='Xonomy.answer(this.val.value); return false'>";
	html += "<input name='val' class='textbox focusme' type='date' style='width: 140px;' value='" + Xonomy.xmlEscape(defaultString) + "' onkeyup='Xonomy.notKeyUp=true'/>";
	html += " <input type='submit' value='OK'>";
	html += "</form>";
	return html;
};

Xonomy.askNumString = function (defaultString, askerParameter, jsMe) {
	var width = ($("body").width() * .5) - 75
	var html = "";
	html += "<form onsubmit='Xonomy.answer(this.val.value); return false'>";
	html += "<input name='val' class='textbox focusme' type='number' style='width: " + width + "px;' value='" + Xonomy.xmlEscape(defaultString) + "' onkeyup='Xonomy.notKeyUp=true'/>";
	html += " <input type='submit' value='OK'>";
	html += "</form>";
	return html;
};
